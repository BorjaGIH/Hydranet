...
def hydranet_loss(concat_true, concat_pred):
    r_loss = regression_loss(concat_true, concat_pred) 
    c_loss = categorical_classification_loss(concat_true, concat_pred)
    h_loss = r_loss + c_loss
    return h_loss
...
'''def make_tarreg_loss(ratio=1., hydranet_loss_=hydranet_loss): # Targeted regularization loss
    ...
    def tarreg_ATE_unbounded_domain_loss(concat_true, concat_pred):
        ...
        inv_t_pred_0 = tf.math.divide(tf.constant(1.), t_pred[:, 0])
        inv_t_pred_1 = tf.math.divide(tf.constant(1.), t_pred[:, 1])
        inv_t_pred_2 = tf.math.divide(tf.constant(1.), t_pred[:, 2])
        inv_t_pred_3 = tf.math.divide(tf.constant(1.), t_pred[:, 3])
        inv_t_pred_4 = tf.math.divide(tf.constant(1.), t_pred[:, 4])
        ...
        h = tf.transpose(
            [tf.cast(t_true == 1, tf.float32) * inv_t_pred_1 - tf.cast(t_true == 0, tf.float32) * inv_t_pred_0,
             tf.cast(t_true == 2, tf.float32) * inv_t_pred_2 - tf.cast(t_true == 0, tf.float32)  * inv_t_pred_0,
             tf.cast(t_true == 3, tf.float32) * inv_t_pred_3  - tf.cast(t_true == 0, tf.float32)  * inv_t_pred_0,
             tf.cast(t_true == 4, tf.float32) * inv_t_pred_4 - tf.cast(t_true == 0, tf.float32)  * inv_t_pred_0])
